  ...
  std::vector<std::shared_ptr<Light> > & lights)
{
    ...
    std::unordered_map<std::string,std::shared_ptr<Material> > & materials)
  {
    ...
    for(const json & jmat : j)
    {
      ...
      // For refraction
      material->is_dielectric = jmat["is_dielectric"];
      material->n_t = jmat["n_t"];
      material->a_rgb = parse_Vector3d(jmat["a_rgb"]);
    ...
    std::vector<std::shared_ptr<Light> > & lights)
  {
    ...
    for(const json & jlight : j)
    {
      ...
      }else if(jlight["type"] == "point")
      {
        ...
        light->I = parse_Vector3d(jlight["color"]);
        lights.push_back(light);
      }else if(jlight["type"] == "area")
      {
        std::shared_ptr<AreaLight> light(new AreaLight());
        light->c = parse_Vector3d(jlight["position"]);
        light->a = parse_Vector3d(jlight["a"]);
        light->b = parse_Vector3d(jlight["b"]);
        light->I = parse_Vector3d(jlight["color"]);
        lights.push_back(light);
      }
    }
